package VPNDetection::OauthError;

use strict;
use warnings;

use parent 'VPNDetection::Error';
...
sub _from {
    my ($class, %args) = @_;
    my $type = $args{error_code} eq 'access_denied' ? 'VPNDetection::OauthAccessDeniedError'
        : $args{error_code} eq 'expired_token' ? 'VPNDetection::OauthExpiredTokenError'
        : $class;
...
package VPNDetection::OauthAccessDeniedError;

use strict;
use warnings;

our @ISA = ('VPNDetection::OauthError');
our $VERSION = '3.1.0';

package VPNDetection::OauthExpiredTokenError;
...
A L<VPNDetection::Error>, so code catching every failure the client reports still
...
B<VPNDetection::OauthAccessDeniedError>, when the person refused it, and
B<VPNDetection::OauthExpiredTokenError>, when the device code expired or was
