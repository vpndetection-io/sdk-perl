...
my $corpus = VPNDetectionTest::corpus()->{oauth};
...
subtest 'no OAuth request carries the API key' => sub {
    ...
    my $device = $oauth->device_authorization('vpndetection-cli', scope => 'account.read');
    $oauth->metadata;
...
subtest 'each operation requests its endpoint with exactly its form fields' => sub {
    serve({ status => 200, body => \%EVERY_REQUIRED_MEMBER });
    VPNDetection->new(base_url => $origin->url . '/')->oauth->metadata(timeout => 5);
    ...
    serve({ status => 200, body => \%EVERY_REQUIRED_MEMBER });
    client()->oauth->device_authorization('vpndetection-cli', scope => '', resource => '');
    is_deeply(form_fields($requests[0]{body}), { client_id => 'vpndetection-cli' },
...
subtest 'a 2xx that lacks a required member or does not parse is the ordinary error' => sub {
    ...
    for my $name (sort keys %cases) {
        serve($cases{$name});
        my $error = settle(sub { client()->oauth->exchange_device_code('vpndetection-cli', 'mo_dc_x') });
...
subtest 'a poll on the real clock waits before its first request' => sub {
    ...
    client()->oauth->poll_device_token('vpndetection-cli', { %DEVICE, interval => 1 });
...
my %timed = (
    ...
    device_authorization => sub { shift->device_authorization('vpndetection-cli', @_) },
    exchange_device_code => sub { shift->exchange_device_code('vpndetection-cli', 'mo_dc_x', @_) },
    exchange_refresh_token => sub { shift->exchange_refresh_token('vpndetection-cli', 'mo_rt_x', @_) },
    revoke => sub { shift->revoke('vpndetection-cli', 'mo_rt_x', @_) },
    poll_device_token => sub {
        my $oauth = shift;
        fake_clock($oauth);
        $oauth->poll_device_token('vpndetection-cli', \%DEVICE, @_);
    },
);

for my $body (qw(stall_body trickle_body)) {
    subtest "every OAuth call is bounded on a $body, per call and by the client" => sub {
        my $slow = VPNDetectionTest::Origin->new(\&{"VPNDetectionTest::Origin::$body"});
        for my $name (sort keys %timed) {
            for my $bound ([3, timeout => 0.25], [0.25]) {
                ...
                my $client = VPNDetection->new(base_url => $slow->url, retries => 0, timeout => $client_timeout);
                my $oauth = $client->oauth;
                my $started = Time::HiRes::time();
...
subtest 'an option or argument OAuth cannot use is refused before any request' => sub {
    ...
    eval { $oauth->device_authorization('vpndetection-cli', scope => ['account.read']) };
    like($@, qr/expected scope as a string/, 'a scope must be a string');
    eval { $oauth->revoke('vpndetection-cli', 'mo_rt_x', timeout => -1) };
    ...
    eval { $oauth->poll_device_token('vpndetection-cli', 'mo_dc_x') };
...
subtest 'the promise forms chain' => sub {
    ...
    fake_clock($oauth);
    my $got;
    $oauth->device_authorization_p('vpndetection-cli')
        ->then(sub { $oauth->poll_device_token_p('vpndetection-cli', shift) })
        ->then(sub { $got = shift })
        ->wait;
